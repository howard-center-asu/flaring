...
Our analysis relies on three data sets. 
...
We thengrouped by governor and filtered for "Grisham (D)" to get a total number of APDs issued under her administration. 
...
FACT: There are nearly 55,000 active oil and gas wells in New Mexico. 
...
FACT: San Juan, Rio Arriba, and Sandoval counties in northwest New Mexico are home to nearly 20,000 active wells. 

The well data we have does not include an associated county. It does include coordinates which we can join to the same county shapefile we used in our satellite analysis. 

```{r}

# Read  shapefile 
counties <- st_read("dtl_cnty.gdb", options = c("METHOD"="SKIP")) %>%  
